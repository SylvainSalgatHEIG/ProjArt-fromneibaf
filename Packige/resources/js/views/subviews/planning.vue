...
function getDateRange({ date, type = "short" }) {
  ...
  if (type === "short") {
    return {
      start:
        formatTwoDigits(start.getDate()) +
        "." +
        formatTwoDigits(start.getMonth() + 1),
      end:
        formatTwoDigits(end.getDate()) +
        "." +
        formatTwoDigits(end.getMonth() + 1),
    };
  } else if (type === "long") {
    return {
      start:
        ...
        formatTwoDigits(end.getMonth() + 1) +
        "." +
        end.getFullYear().toString().substr(-2),
    };
  }
}
...
<template>
  <div class="content">
    ...
    >
      ...
      <div
        v-for="event of schedule.daysCourse"
        class="planning"
        v-show="event.hasCourses"
      >
        <div
          class="date"
          v-bind:class="
            formatDate(new Date(event.courses[0].date)) ==
            formatDate(new Date(todayDate))
              ? 'currentDay'
              : ''
          "
        >
          {{ event.day }} {{ event.dayTwoDigits }}
        </div>
